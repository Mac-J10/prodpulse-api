...
class PulseModelTest(APITestCase):
    def test_str_method(self):
        ...
        prod = Product.objects.create(title="P1", sku="P1", category=cat)
        pulse = Pulse.objects.create(
            product=prod, timestamp="2025-01-01T00:00:00Z", value=12.3456, unit="u"
        )
...
class PulseAPITest(APITestCase):
    def setUp(self):
        cat = Category.objects.create(name="TestCat", slug="test-cat")
        self.prod = Product.objects.create(title="P1", sku="P1", category=cat)
        self.pulse = Pulse.objects.create(
            product=self.prod, timestamp="2025-01-01T00:00:00Z", value=12.3456, unit="u"
        )
